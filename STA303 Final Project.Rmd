...
Since our main purpose was to test whether or not gender affected applicants' acceptance, this ordinal variable helped us create a response variable that follows Bernoulli distribution.
Some phases did not have information about applicants' gender, so we made all phases to have gender information of all applicants.

In phase 2, we saw that none of the applicants who preferred not to say their gender got accepted and therefore, we omitted them from phase 2. 
...
The proportions of female applicants who got accepted in phase 1 was similar to that of male applicants. On the other hand, among those who preferred not to say their gender, there was 25% difference between applicants who got accepted and who did not. However, this does not provide much evidence that phase 1 was biased for those who preferred not to say their gender, because we only had 11 applicants^[This is only 2% of all applicants.] who preferred not to say their gender and proportion largely depends on the number of applicants.
...
Our response variable followed Bernoulli distribution, which indicated whether or not each applicant passed each phase. Since the logit link was used for Bernoulli response, we could assume that there was a linear relationship between the transformed response and explanatory variables. Also, we assumed that information of each applicants are independent. 
After checking for the assumptions as above, we used generalized linear models.

In initial model, where gender was our only predictor, confidence interval of all genders included 0. Therefore at the 95% confidence level, it was believable that male and female applicants who preferred not to say their gender had the same log odds of being accepted by the AI in phase 1. In addition to this model, we tried to investigate whether the AI was biased with gender and other variables in phase 1 by comparing two models; One with all variables included and another with only gender excluded from the previous model.
...
Lastly, for phase 3, we used a simple linear regression model with a single predictor, gender, for interview ratings. Here, we assumed that interview scores were linearly related to gender. By looking at the residuals and fitted value plot, we checked that the mean of the errors was 0. Also, standardized residuals and fitted value graph showed us the errors were homoscedastic. Lastly, the errors were uncorrelated.
...
\begin{table}[h]
\caption{Partial Table of Deviances and P-values for Gender in the Phase 1 and 2}
\label{tab:my-table}
\centering

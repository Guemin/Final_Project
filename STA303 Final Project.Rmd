...
- In the first and hiring phase assessed by AI, the proportion of applicants who got approved to next phase was approximately evenly split between male and female.
- In the third hiring phase, only 7% of the applicants were invited to an interview and among them, the averages of grades of interview between male and female were similar.
...
Also, we see that salary significantly depends on the person's role seniority level. In particular, manager, director, vice president receive salaries significantly different from other positions (see Figure 2). 

We also looked at a boxplot for salary by gender to see whether there is a gender difference in salary (see Appendix Figure 1). Although gender did not seemed to create sysemtatic difference, the one-way anova of gender as a predictor and salary as a response suggested a significant difference. However, we needed to see this along with other variables related to an employee's value to the company. Thus, we investigated whether a regression model with and without gender as a predictor significantly differs after we fit each model and make comparison.
...
my_plot_salary_by_gender <- black_saber_current_employees_cleaned %>% 
  ...
  geom_boxplot() +
  theme_minimal() +
  labs(x = "Gender", y= "Salary($)", caption = "Figure 1A: Boxplots for Salary by Seniority Role") +
  theme(plot.caption = element_text(hjust = 0))
...
We chose a linear mixed effects model for the regression model because financial quarter will also produce sysematic difference, since for instance, in financial quarters where Black Saber earns a lot of money, employees will receive bonuses. However, we were not interested in this variable because we only cared about the employee's value. Further, statistically speaking, employees' salaries were sampled across various financial quarter and so this would violate the independence assumption for observations. Thus, it was important to add financial quarter as a random intercept.
...
```
Note that we also tested whether we should include team as predictor for salary becuase it might be ambiguous of whether team had fixed effect on salary based on the above plot as compared to seniority of role. The result showed that we should also include team.
```{r}
...
Since our main purpose was to test whether or not the gender of applicants affected the acceptance of the phase, this ordinal variable helped us create a response variable that follows Bernoulli distribution.
...
In phase 2, we could see that none of the applicants who preferred not to say about their gender got accepted and therefore, we omitted them from phase 2. 
...
The proportions of applicants who were accepted in phase 1 had similar proportion between male and female. However, there is 25% difference in the the accepted rate in phase 1 with the applicants who prefered not to say their gender.
This difference makes sense since proportion depends on the number of the applicants largely. We have only 11 Prefer not to say applicants, which is 2% of whole applicants. 

In phase2, we can see male has slightly higher approved proportion(10%) within male applicants, which is about 6% higher than female.

![](images/hiring_boxplot_gender.png)

In phase 3, the ranges of interview scores given by interviewer 1 and 2 were similar to each other. Although there was a slight difference between mean scores of male and female who were assessed by interviewer 2, it was only a small difference of 2.5 in scores; Therefore, we could assume that gender did not influence the interview score.
...
Response variable followed Bernoulli distribution, which indicates whether each applicant passed each phase or not. Since the logit link was used for Bernoulli response, we could assume that there was a linear relationship between the transformed response and explanatory variables. Also, We could assume that information of each applicants are independent. 
After checking for the assumptions as above, we used generalized mixed model.

In initial model, where gender was our only predictor, confidence interval of all genders included 0. Therefore at the 95% confidence level, it was believable that male and female applicants who preferred not to say about their gender had the same log odds of being accepted by AI in phase 1. In addition to this model, we tried to investigate whether the AI was biased with gender and other variables in phase 1 by comparing two models; One with all variables included and another with gender excluded from the first model.
...
Lastly, for phase 3, we used a simple linear regression model. We had one predictor, gender, for interview ratings. we assumed that interview scores were related to gender linearly. By residuals and fitted value plot we could see that the mean of the errors was 0. Also, standardized residuals and fitted value graph showed us the errors were homoscedastic. Lastly, the errors were uncorrelated.

**\underline{Results}**

This table shows the deviance and p-value for model 1 which excluded gender and model 2 which include all variables:

\begin{table}[h]
\caption{Partial Table of Deviance and P-value for Gender in the Phase 1 and 2}
\label{tab:my-table}
\centering
...
The deviance tests using the two models told us that it is not preferable to add gender as a predictor variable to the models. The deviance difference between models in phase 1 with and without gender was 1.21, which was pretty small. Also, the large p-value supported that adding a new indicator variable, gender, to the model with all the other variables included was not very helpful in predicting the acceptance of an applicant.

Similarly, we could see that there were small deviance(0.62) and large p-value(0.43) in deviance test for phase 2 models. Therefore adding gender to the model did not explain the acceptance of applicants in phase 2. 

From confidence interval in phase 3, we could see that the coefficients of gender contained 0.^[The full table of the 95 % confidence interval for the model is included in the Appendix] Therefore, we concluded that interviewers were not biased with gender. In other words, gender did not affect interview scores.
...
In particular, we found that salary will be affected by gender when other explanatory variables such as productivity, leadership, and team are controlled.
...
In hiring process, we can find that both AI and human are not biased with gender. Specifically, we found that applicants' acceptance will not be affected by gender assessed by AI. Also, the interview score will not be affected by gender when applicants are evaluated by interviewers in phase 3.
